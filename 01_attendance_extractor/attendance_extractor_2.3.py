...
class AttendanceMasterGenerator:
    ...
    def find_date_column(self, ws, header_row: int, date_str: str) -> Optional[int]:
        ...
        return None
    def extract_clean_comment(self, raw_comment: str) -> str:
        """
        Extract the actual comment text from Excel threaded comment format.
        
        Args:
            raw_comment: Raw comment text from Excel
        ...
        if "Comment:" in raw_comment:
            # Split by "Comment:" and take everything after it
            parts = raw_comment.split("Comment:", 1)
            if len(parts) > 1:
        ...
        # If no "Comment:" marker, return as-is (for old-style comments)
        return raw_comment.strip()
    
    
    
    def get_attendance_value_and_comment(self, attendance_cell, id_val) -> Tuple[str, str]:
        """Extract attendance value and comment from a cell."""       
        attendance_str = '-'
        comment_text = ''
        
        if id_val.startswith(self.id_prefix):
            if attendance_cell.value:
                attendance_str = str(attendance_cell.value).strip().upper()
                if attendance_str not in self.valid_values:
                    attendance_str = '-'
            
            if attendance_cell.comment:
                comment_text = attendance_cell.comment.text
                if comment_text:
                    comment_text = comment_text.strip()
        
        return attendance_str, self.extract_clean_comment(comment_text)
    ...
    def extract_records_from_source(self, source_file: Path) -> List[Dict]:
        ...
        try:
            ...
            for row_idx in range(header_row + 1, ws.max_row + 1):
                ...
                attendance_cell = ws.cell(row_idx, attendance_col)
                ...
                attendance_val, comment_text  = self.get_attendance_value_and_comment(attendance_cell, id_val)
    ...
    def create_master_sheet(self, all_records: List[Dict]):
        ...
        try:
            ...
            # Write header row
            headers = ["ID# Assigned", "Region", "Naqeebs", self.attendance_date, "Comments"]
            ws.append(headers)
            ...
            for record in all_records:
                ws.append([
                    ...
                    record['naqeeb'],
                    record['attendance'],
                    record.get('comment', '')  # Use .get() for backward compatibility
                ])
